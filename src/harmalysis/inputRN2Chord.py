...
def inputRN (RN):
    ...
    try:
        roman = harmalysis.parsers.roman.parse(RN)
        chordlabel = harmalysis.parsers.chordlabel.parse(str(roman.chord))
        output = {'Current key' : str(roman.main_key),
            'Tonicized key' : str(roman.secondary_key),
            'Intervallic construction' : str(roman.chord),
            'Inversion' : str(roman.chord.inversion),
            'Chord label' : chordlabel,
            'Default function' : roman.chord.default_function,
            'Chord pitches' : str(roman.chord.get_pitch_spellings()),
            'Chord pitch classes' : str(roman.chord.get_pitch_classes())
            }
        main_key = roman.main_key
    except lark.exceptions.UnexpectedCharacters:
        #pass
