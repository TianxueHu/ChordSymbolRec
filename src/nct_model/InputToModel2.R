...
filename <- "op20n2-04_mel2_features.krn"                                                 ######CHANGE FILE!!
...
raw <- "score_reduced1.krn"                                                                                #####CHANGE FILE
...
for(i in 1:nrow(dataframe)){ #for loop counting dataframe
  if (dataframe$Response[i] == 0){
    ...
    note<-rawfile$two[num]                                                                               ####CHANGE COLUMN!!
    tmp<-str_remove_all(note,"[^0-9.]")
    rawfile$two[num]<- paste(tmp,"r",sep='')                                                            ####CHANGE COLUMN!!
  }
}
...
write.table(rawfile, file = "score_reduced2.krn", sep = "\t",
