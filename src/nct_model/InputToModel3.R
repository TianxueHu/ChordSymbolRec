...
filename <- "op20n2-04_mel3_features.krn"                                                 ######CHANGE FILE!!
...
raw <- "score_reduced2.krn"                                                                                #####CHANGE FILE
...
for(i in 1:nrow(dataframe)){ #for loop counting dataframe
  if (dataframe$Response[i] == 0){
    ...
    note<-rawfile$three[num]                                                                               ####CHANGE COLUMN!!
    tmp<-str_remove_all(note,"[^0-9.]")
    rawfile$three[num]<- paste(tmp,"r",sep='')                                                            ####CHANGE COLUMN!!
  }
}
...
write.table(rawfile, file = "score_reduced3.krn", sep = "\t",
